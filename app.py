    ...
    def check_card(self, cc, mm, yy, cvv):
        ...
        try:
            ...
            if r1.status_code != 200:
                return "DECLINED", "Cart failed"
            
            cart_json = r1.json()
            if 'redirectUrlPath' not in cart_json:
                return "DECLINED", "No cart token"
            
            match = re.search(r'cartToken=([^&]+)', cart_json['redirectUrlPath'])
            if not match:
                return "DECLINED", "Cart token error"
...
def check_card_api():
    ...
    if '|' not in cc_param:
        return jsonify({
            'status': 'ERROR',
            'message': 'Use format: cc=number|mm|yy|cvv'
        })
    ...
    status, message = checker.check_card(cc, mm, yy, cvv)
    
    # Return result
    return jsonify({
        'status': status,
        'message': message,
        'card': f"{cc[:6]}XXXXXX{cc[-4:]}"
    })
...
def home():
    return jsonify({
        ...
        'usage': '/check?key=BlackXCard&cc=card|mm|yy|cvv&proxy=host:port:user:pass',
        'example': '/check?key=BlackXCard&cc=4242424242424242|12|25|123'
    })
